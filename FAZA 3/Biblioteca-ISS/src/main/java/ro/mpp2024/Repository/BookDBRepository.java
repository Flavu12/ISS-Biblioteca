...
public class BookDBRepository implements IBookRepository{
    ...
    @Override
    public Collection<Book> findAvailableBooks() {
        Collection<Book> availableBooks = new ArrayList<>();
        try (Connection connection = jdbcUtils.getConnection();
             PreparedStatement findStmt = connection.prepareStatement("select * from books where disponibilitate = ?")) {
            findStmt.setBoolean(1, true);
            ResultSet resultSet = findStmt.executeQuery();
            while (resultSet.next()) {
                Integer id = resultSet.getInt("id");
                String titlu = resultSet.getString("titlu");
