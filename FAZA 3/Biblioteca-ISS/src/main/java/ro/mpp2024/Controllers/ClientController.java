...
public class ClientController implements Observer<Event> {
    ...
    private void initModelAllBooks(){
        services.findAvailableBook().stream().forEach(System.out::println);
        modelAllBooks.setAll(services.findAvailableBook());
    }
    ...
    private void handleAddOrderBook(){
        ...
        try{
            ...
            OrderItem orderItem = new OrderItem(new Pair<>(book.getID(), null), book.getTitlu(), book.getId_exemplar(), book, null);
    ...
    private void handleAddOrder() {
        Integer id_exemplar = 0;
        for (OrderItem orderItem : modelOrderItems) {
            id_exemplar += orderItem.getId_exemplar();
        }
        try {
            Order order = new Order(id_exemplar, rootUser.getID(), Status.PENDING);
            ...
            // Schimbă disponibilitatea cărților din true în false pentru cărțile din comandă
            for (OrderItem orderItem : modelOrderItems) {
                Book book = orderItem.getBook();
                book.setDisponibilitate(false);
                services.updateBook(book); // Actualizează starea cărții în baza de date
            }

            // Șterge elementele din lista de comenzi după ce comanda este dată
            modelOrderItems.clear();
        } catch (RuntimeException ex) {
            MessageAlert.showErrorMessage(mainStage, ex.getMessage());
        }
    }
